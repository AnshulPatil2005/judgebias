// scrape_allahabad.js
// Full rewrite: reliable Case Type selection (#case_type44), year-based downloads,
// PDFs + Excel export, robust selectors, frame-aware, popup handling.
//
// Run:
//$env:CASE_YEAR = "2018"
//$env:CASE_TYPE = "A227(MATTERS)"
//node scrape_allahabad.js
//
// Notes:
// - CASE_YEAR controls the Year field AND saves to downloads/<YEAR>/
// - CASE_TYPE is optional (exact text or regex). If omitted, first valid option is chosen.
...
// --------------------- Dropdown helpers -----------
async function selectFirstUsable(dropdown, label) {
  ...
  const ok = await dropdown.evaluate((el, v, t) => {
...
async function selectByRegexOrFirst(dd, wanted) {
  ...
  if (ok) console.log(`Case Type: selected "${match.text || match.value}"`);
  return ok;
}
...
async function selectBench(scope) {
  const page = owningPage(scope);
  const scopes = [scope, page, ...page.frames()];
  // wait until bench dropdown gets options after HC selection
...
// ----------- Case Type: #case_type44 (robust & blocking until populated)
async function selectCaseType44(page, wantedLabel = TARGET_CASE_TYPE) {
  ...
  // Try wanted regex/text; else first usable
  ...
  for (const s of [page, ...page.frames()]) {
    try {
      const data = await s.evaluate(() => {
        ...
        // IA table
        const iaHdr = cells.find(el => /IA Details/i.test(norm(el.textContent)));
        const ia = [];
        if (iaHdr) {
...
function saveExcel(rows, base = 'case_details') {
  ...
  const cols = [
    'viewButtonIndex','filingNumber','filingDate','registrationNumber','registrationDate','cnrNumber',
    'firstHearingDate','decisionDate','caseStatus','natureOfDisposal','coram','benchType','judicialBranch',
    'state','district','petitionerAndAdvocate','respondentAndAdvocate','underActs','underSections','category','subCategory','iaDetails'
  ];
...
(async function run() {
  ...
  // High Court + Bench
  // select HC via sess_state_code if present; else try other selectors
  ...
  if (await sess.count()) {
    try {
      ...
      await selectByRegexOrFirst(sess, HIGH_COURT_OPTION_REGEX.source);
    } catch {}
  } else {
  ...
  // >>> Critical: select case type (#case_type44) AFTER radio + year
  await selectCaseType44(page, TARGET_CASE_TYPE);
  await dismissVisibleOk(page);
  await wait(HUMAN_DELAY);

  // Wait for results table (after you solve captcha + Go)
